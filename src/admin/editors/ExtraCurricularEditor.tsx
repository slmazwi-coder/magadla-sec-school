...
const categories: Activity['category'][] = ['Academic', 'Culture', 'Sport'];
		...
		<div>
			<div className="flex items-center justify-between mb-8">
				...
				<button
					...
					className="flex items-center gap-2 bg-school-green text-white px-4 py-2 rounded-xl font-medium hover:opacity-90"
				>
				...
				<div className="bg-gray-800 border border-gray-600 rounded-2xl p-6 mb-8">
					...
					<div className="space-y-4">
						...
						<select
							...
							onChange={(e) => setEditing({ ...editing, category: e.target.value as Activity['category'] })}
							className="w-full bg-gray-700 border border-gray-600 rounded-xl px-4 py-2 text-white"
						>
							{categories.map((c) => (
						...
						<div className="pt-4 border-t border-gray-700 flex flex-wrap items-center justify-between gap-4">
							<button
								onClick={save}
								disabled={isScanning}
								className="flex items-center gap-2 bg-school-green text-white px-6 py-2 rounded-xl font-medium hover:opacity-90 disabled:opacity-50"
							>
								{isScanning ? (
									<>
					...
					{items.map((item) => (
						<div key={item.id} className="bg-gray-800 border border-gray-700 rounded-xl p-4 flex gap-4">
							...
							<div className="flex flex-col gap-1 shrink-0">
								<button onClick={() => { setEditing(item); setIsNew(false); }} className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-white">
									<Pencil size={14} />
								</button>
								<button onClick={() => remove(item.id)} className="p-1.5 hover:bg-gray-700 rounded text-gray-400 hover:text-red-400">
									<Trash2 size={14} />
								</button>
							</div>
						</div>
